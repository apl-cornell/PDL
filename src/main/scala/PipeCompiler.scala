...
//   * @return
//   */
//  def toStageCmds(c: Command, context: Option[Expr], nextStage: Channel, channelMap: Map[Id,Channel]): List[StageCommand] = c match {
//    case CSeq(c1, c2) => toStageCmds(c1, context, nextStage, channelMap) ++ toStageCmds(c2, context, nextStage, channelMap)
//    case CIf(cond, cons, alt) =>
//      val ncont = context match {
//        case Some(ctx) => Some(AndOp (cond, ctx))
//        case None => Some(cond)
//      }
//      List(SIf(cond, toStageCmds(cons, ncont, nextStage, channelMap), toStageCmds(alt, ncont, nextStage, channelMap)).setPos(c.pos))
//    case CAssign(lhs, rhs) => List(SAssign(lhs, rhs).setPos(c.pos))
//    case CRecv(lhs, rhs) => rhs match {
//        case EMemAccess(mem, index) => {
//          val destChannel = channelMap(mem)
//          val msg = MemoryRead(index)
//          val msgName = EVar(freshVar("memread"))
//          val msgExpr = SAssign(msgName, msg)
//          List(msgExpr, SSend(context, msgName, destChannel))
//          //TODO generate the corresponding receive in the next stage
//        }
//      }
//    case CCall(id, args) =>
//    case COutput(exp) =>
//    case CReturn(exp) =>
//    case CExpr(exp) =>
//    case CDecl(id, typ, thisCycle) => List()
//    case Syntax.CEmpty => List()
//  }
//
//
//  /**
...
//      val (renameTcmds, tmap) = liftCommandsFromIf(tcmds, renameMap, cond, "_t")
//      val fcmds = flattenCommands(fbr)
//      val notCond = EUop(NotOp().setPos(cond.pos), cond).copyMeta(cond)
//      val (renameFCmds, fmap) = liftCommandsFromIf(fcmds, renameMap, notCond, "_f")
...
//   * @return
//   */
//  def liftCommandsFromIf(cs: List[Command], renameMap: Map[Id, EVar], cond: Expr, suffix: String): (List[Command], Map[Id, EVar]) = {
//    var nmap = renameMap
...
//      case CDecl(_, _, _) => l //TODO is this the right point to eliminate these?
//      //we excpect no IF or TBar commands
//    })
//    (ncs, nmap)
//  }
//  /**
...
//    case eu@EUop(_, ex) => eu.copy(ex = renameVars(ex, renameMap)).copyMeta(e)
...
//    case EUop(_, ex) => getReadVarsE(ex)
