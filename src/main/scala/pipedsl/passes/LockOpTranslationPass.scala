...
import pipedsl.common.Utilities.{flattenStageList, lock_handle_prefix, mkOr, updateListMap}
...
*/ override def run(p: Prog): Prog =
  ...
  private def modifyMemArg(e: Expr, isLhs: Boolean): Expr = {

    e match {
      //SimplifyRecvPass ensures that the index expression is always a variable
      case em@EMemAccess(mem, idx@EVar(_), wm, _, _, false) if isLockedMemory(mem) =>
        val l_arg = LockArg(mem, em.granularity match
        { case Locks.Specific => Some(idx)
          case Locks.General => None
        })
        val newArg: Expr = idx
        val res = EMemAccess(mem, newArg, wm,
          Some(lockVar(l_arg, LockedMemState.Acquired)),
          Some(lockVar(l_arg, LockedMemState.Operated)), isAtomic = false).setPos(em.pos)
        res.copyMeta(em)
