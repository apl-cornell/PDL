...
object SimplifyRecvPass extends CommandPass[Command] with ModulePass[ModuleDef] with ProgPass[Prog] {
  ...
  private def runHelper(c:Command): Command = c match {
    ...
    case CRecv(lhs, rhs, isAtomic) => (lhs, rhs) match {
      case (EVar(_), EMemAccess(_, EVar(_),_, _, _)) => c //leave it alone, already in the form we want
      case (EVar(_), EMemAccess(mem, idx, wm, inHandle, outHandle)) => //separate out the index computation
        val idxAssgn = CAssign(newVar("index", idx.pos, idx.typ), idx).setPos(idx.pos)
        val access = EMemAccess(mem, idxAssgn.lhs, wm, inHandle, outHandle).setPos(rhs.pos)
        access.typ = rhs.typ
        access.portNum = rhs.portNum
        CSeq(idxAssgn, CRecv(lhs, access, isAtomic)).setPos(c.pos)
      case (EMemAccess(_, EVar(_), _, _, _), EVar(_)) => c //leave it alone, already in form we want
      case (EMemAccess(_,EVar(_), _, _, _), _) => // separate out the RHS computation
        val rhsAssgn = CAssign(newVar("msg", rhs.pos, rhs.typ), rhs).setPos(rhs.pos)
        CSeq(rhsAssgn, CRecv(lhs, rhsAssgn.lhs, isAtomic).setPos(c.pos)).setPos(c.pos)
      case (EMemAccess(mem,idx,wm, inHandle, outHandle), EVar(_)) => //separate out the index computation
        val idxAssgn = CAssign(newVar("index", idx.pos, idx.typ), idx).setPos(idx.pos)
        val access = EMemAccess(mem, idxAssgn.lhs, wm, inHandle, outHandle).setPos(lhs.pos)
        access.typ = lhs.typ
        access.portNum = lhs.portNum
        CSeq(idxAssgn, CRecv(access, rhs, isAtomic).setPos(c.pos)).setPos(c.pos)
      case (EMemAccess(mem,idx, wm, inHandle, outHandle), _) => //separate the index computation AND the rhs computation into new variables
        val idxAssgn = CAssign(newVar("index", idx.pos, idx.typ), idx).setPos(idx.pos)
        val rhsAssgn = CAssign(newVar("msg", rhs.pos, rhs.typ), rhs).setPos(rhs.pos)
        val access = EMemAccess(mem, idxAssgn.lhs, wm, inHandle, outHandle).setPos(lhs.pos)
        access.typ = lhs.typ
        ...
        CSeq(idxAssgn,
          CSeq(rhsAssgn,
            CRecv(access, rhsAssgn.lhs, isAtomic).setPos(c.pos))
            .setPos(c.pos))
          .setPos(c.pos)
