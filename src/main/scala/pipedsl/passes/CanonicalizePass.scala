...
class CanonicalizePass() extends CommandPass[Command] with ModulePass[ModuleDef]
  with FunctionPass[FuncDef] with ProgPass[Prog]
  {
    ...
    override def run(p: Prog): Prog = p.copy(moddefs = p.moddefs.map(m => run(m)), fdefs = p.fdefs.map(f => run(f))).setPos(p.pos)
    ...
    def extractCastVars(c: Command): Command = c match
    {
      ...
      case CAssign(lhs, rhs, isAtomic) => val (nrhs, nassgns) = extractCastVars(rhs)
        val nc = CAssign(lhs, nrhs, isAtomic).setPos(c.pos)
        CSeq(nassgns, nc).setPos(c.pos)
      case CRecv(lhs, rhs, isAtomic) => val (nrhs, na1) = extractCastVars(rhs)
        val (nlhs, na2) = extractCastVars(lhs)
        val nassgns = CSeq(na1, na2).setPos(c.pos)
        CSeq(nassgns, CRecv(nlhs, nrhs, isAtomic).setPos(c.pos)).setPos(c.pos)
