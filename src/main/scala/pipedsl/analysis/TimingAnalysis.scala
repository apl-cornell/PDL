...
class TimingAnalysis(program: Tree[ProgramNode, Prog], typeAnalysis: TypeAnalysis) extends Attribution{
  ...
  //If previous, just take availableNow of previous + the after
  //If previous is part of CTBar, we want the previous + the available made f
  ...
  /** 
   * Attribute that gives the available variable generated by a node
   */
  val availableNowAfterNode: Available => ProgramNode => Available = {
    paramAttr {
      avail => {
        case CSeq(c1, c2) => val a1 = availableNowAfterNode(avail)(c1); availableNowAfterNode(a1)(c2)
        case CTBar(c1, c2) => val a1 = availableNowAfterNode(avail)(c1); availableNowAfterNode(a1 ++ availableNextAfterNode(NoneAvailable)(c1))(c2)
        case CAssign(lhs@EVar(id), rhs, typ) => avail + id
        case CIf(cond, cons, alt) => availableNowAfterNode(avail)(cons).intersect(availableNowAfterNode(avail)(alt))
        case CSplit(cases, default) =>
          var runningAvail: Available = availableNowAfterNode(avail)(default)
          for (c <- cases) {
            runningAvail = runningAvail.intersect(availableNowAfterNode(avail)(c))
          }
          runningAvail
        case _ => avail
      }
  ...
  val availableNextAfterNode: Available => ProgramNode => Available = {
    paramAttr {
      avail => {
        case CSeq(c1, c2) => val a1 = availableNextAfterNode(avail)(c1); availableNextAfterNode(a1)(c2)
        case CTBar(c1, c2) => availableNextAfterNode(NoneAvailable)(c2)
        case CRecv(lhs, rhs, typ) =>
          (lhs, rhs) match {
        ...
        case CIf(cond, cons, alt) => availableNextAfterNode(avail)(cons).intersect(availableNextAfterNode(avail)(alt))
        case CSplit(cases, default) =>
          var runningAvail: Available = availableNextAfterNode(avail)(default)
          for (c <- cases) {
            runningAvail = runningAvail.intersect(availableNextAfterNode(avail)(c))
          }
          runningAvail
        case _ => avail
      }
  ...
  val availableNext: ProgramNode => Available = 
    attr {
      case program.prev(program.parent(c@CSeq(c1, c2))) => availableNext(c1) ++ availableNextAfterNode(NoneAvailable)(c1)
