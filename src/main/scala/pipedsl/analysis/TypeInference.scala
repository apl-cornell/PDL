...
import pipedsl.common.Errors.{ArgLengthMismatch, UnexpectedSubtype, UnexpectedType, UnificationError}
...
object TypeInference {
  ...
  def checkProgram(p: Prog): Environment[Id, Type] = {
    val funcEnvs = p.fdefs.foldLeft[Environment[Id, Type]](TypeEnv())((env, f) => checkFunc(f, env.asInstanceOf[TypeEnv]))
    val modEnvs = p.moddefs.foldLeft[Environment[Id, Type]](funcEnvs)((env, m) => checkModule(m, env.asInstanceOf[TypeEnv]))
