...
import pipedsl.analysis.{PredicateAnalysis, TypeAnalysis}
...
class LockConstraintChecker(val predicateAnalysis: PredicateAnalysis, lockMap: Map[Id, Set[LockArg]], lockTypeMap: Map[Id, Map[Id, LockType]], val ctx: Z3Context)
    ...
    c match {
      ...
      case CSplit(cases, default) =>
        ...
        for (caseObj <- cases) {
          ...
          val tenv = ConditionalLockEnv(newEnv.getMappedKeys()
            .foldLeft[Map[LockArg, Z3AST]](Map())((nenv, id) => nenv + (id -> mkImplies(ctx, predicateAnalysis.predicate(caseObj.body), newEnv(id)))),
            ctx)
          if (first) {
            runningEnv = tenv
          } else {
        ...
        val tenv = ConditionalLockEnv(defEnv.getMappedKeys()
          .foldLeft[Map[LockArg, Z3AST]](Map())((nenv, id) => nenv + (id -> mkImplies(ctx, mkAnd(ctx, predicateAnalysis.predicate(default)), defEnv(id)))),
          ctx)
      ...
      case CIf(expr, cons, alt) =>
        ...
        val fenv = ConditionalLockEnv(lf.getMappedKeys()
          .foldLeft[Map[LockArg, Z3AST]](Map())((nenv, id) => nenv + (id -> mkImplies(ctx, predicateAnalysis.predicate(alt), lf(id)))),
          ctx)
        //Merge the two envs
      ...
      case _: CSpeculate =>
        //TODO
        env
      case CAssign(_, rhs, typ) => checkExpr(rhs, env, predicateAnalysis.predicate(c))
      case CRecv(lhs, rhs, typ) => (lhs, rhs) match {
        case (EMemAccess(mem, expr), _) =>
          checkAcquired(mem, expr, env, predicateAnalysis.predicate(c))
        case (_, EMemAccess(mem, expr)) =>
          checkAcquired(mem, expr, env, predicateAnalysis.predicate(c))
        case (_, ECall(mod, _)) =>
          //TODO Maybe just from null
          checkAcquired(mod, null, env, predicateAnalysis.predicate(c))
      ...
      case c@CLockOp(mem, op) =>
        ...
        checkState(mem, env, predicateAnalysis.predicate(c), expectedLockState.order) match {
          case Z3Status.UNSATISFIABLE =>
            env.add(mem, mkImplies(ctx, predicateAnalysis.predicate(c), makeEquals(mem, op)))
