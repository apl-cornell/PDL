...
object Syntax {
  ...
  def isLockedMemory(mem: Id): Boolean = mem.typ.get match { case _:TMemType => false; case _:TVolatileMemType => false; case _ => true }
  ...
  case class CExcept(args: List[Expr]) extends Command  // throw(args...)
  ...
  // Internal commands for exception handling (generated by ExnTranslationPass)
  case class IAbort(mem: Id) extends InternalCommand         // Reset uncommitted lock/memory state
  case class IFifoClear() extends InternalCommand            // Clear all pipeline FIFOs
  case class ICheckExn() extends InternalCommand             // Check global exception flag (guard condition)
  case class ISpecClear() extends InternalCommand            // Clear speculation table
  case class ISetGlobalExnFlag(state: Boolean) extends InternalCommand  // Set/unset gef
  ...
  // Exception block: the except(...): handler for pipeline exceptions
  sealed trait ExceptBlock extends Positional with HasCopyMeta {
    ...
    def get: Command
    def args: List[Id]
    def copyMeta(other: ExceptBlock): this.type = { this.setPos(other.pos); this }
  }
  ...
  case class ExceptEmpty() extends ExceptBlock {
    ...
    override def args: List[Id] = Nil
    override def get: Command = throw new NoSuchElementException("ExceptEmpty.get")
  }

  case class ExceptFull(exn_args: List[Id], c: Command) extends ExceptBlock {
    override def map(f: Command => Command): ExceptBlock = ExceptFull(exn_args, f(c)).copyMeta(this)
  ...
  case class ModuleDef(
    ...
    ret: Option[Type],
    body: Command,
    commit_blk: Option[Command] = None,
    except_blk: ExceptBlock = ExceptEmpty())
    extends Definition with RecursiveAnnotation with SpeculativeAnnotation with ExceptionAnnotation with HasCopyMeta
    {
      ...
      def command_map(f: Command => Command): ModuleDef =
        copy(body = f(body), commit_blk = commit_blk.map(f), except_blk = except_blk.map(f))
      ...
      def hasExceptions: Boolean = except_blk match {
        case _: ExceptEmpty => false
  ...
  val is_excepting_var: Id = Id("__excepting").setType(TBool())
  ...
  case class CirMem(elemTyp: Type, addrSize: Int, numPorts: Int, isVolatile: Boolean = false) extends CirExpr
  case class CirRegFile(elemTyp: Type, addrSize: Int, isVolatile: Boolean = false) extends CirExpr
  case class CirRegister(elemTyp: Type, initVal: Int, isVolatile: Boolean = false) extends CirExpr
